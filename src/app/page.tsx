...
import { Activity, CalendarClock, ChevronRight, ClipboardPlus, Plus, ShieldCheck } from "lucide-react";
...
export default async function DashboardPage() {
  ...
  const [totalCattle, totalGoats, upcomingTreatments, recentlyAddedAnimals, activeAnimals] = await Promise.all([
  ...
  const stats = [
    ...
    { label: "Due Soon", value: upcomingTreatments.length, detail: "next 30 days", icon: CalendarClock, tone: "bg-field-straw text-field-soil" },
        ...
        <div className="grid gap-6 p-5 sm:p-7 lg:grid-cols-[1.1fr_0.9fr] lg:p-8">
          <div className="flex min-h-56 flex-col justify-between gap-8">
            <div className="space-y-4">
              ...
              <div className="space-y-3"><h1 className="max-w-2xl text-3xl font-black leading-tight sm:text-5xl">Livestock records that are quick enough for field work.</h1><p className="max-w-xl text-sm leading-6 text-white/75 sm:text-base">Track cattle and goats, keep treatment reminders visible, and jump into each animal record without digging.</p></div>
            </div>
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:content-end">
            {stats.map((stat) => {
